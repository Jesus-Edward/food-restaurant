...
class ChatController extends Controller
{
    ...
    {
        $request->validate([
            'admin_message' => ['required', 'max:1000'],
        ...
        $chat->message = $request->admin_message;
        $chat->save();

        $avatar = asset(auth()->user()->avatar);
        $senderId = auth()->user()->id;
        broadcast(new ChatEvent($request->admin_message, $avatar, $request->receiver_id, $senderId))->toOthers();
