...
class DailyOfferDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->addColumn('action', function($query){
            $edit = '<a href="'.route('admin.daily-offer.edit', $query->id).
            '" class="btn btn-primary"><i class="fas fa-edit"></i></a>';
            $delete = '<a href="'.route('admin.daily-offer.destroy', $query->id).
            '" class="btn btn-danger ml-2 delete-item"><i class="fas fa-trash"></i></a>';

            return $edit.$delete;
        })
            ->addColumn('image', function($query){
                return '<img width="80px" height="60px" src="'.asset($query->product->thumb_image).'">';
        })
