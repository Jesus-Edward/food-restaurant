...
@section('og_metatag_section')
    ...
    <meta property="og: description" content="{{ truncateTitle(@$blog->seo_post) }}">
            ...
            <div class="container">
                <div class="fp__breadcrumb_text">
                    ...
                    <ul>
                        ...
                        <li><a href="javascriptt:;">blog details</a></li>
                    </ul>
                </div>
            </div>
        </div>
                ...
                <div class="col-xl-8 col-lg-8">
                    <div class="fp__blog_det_area">
                        <div class="fp__blog_details_img wow fadeInUp" data-wow-duration="1s">
                            <img src="{{ asset($blog->image) }}" alt="{{ asset($blog->title) }}" class="imf-fluid w-100">
                        </div>
                            ...
                            <div class="blog_tags_share d-flex flex-wrap justify-content-between align-items-center">
                                ...
                                <div class="share d-flex flex-wrap align-items-center">
                                    ...
                                    <ul class="d-flex flex-wrap">
                                        ...
                                        <li><a href="
                                            http://twitter.com/share?text={{ $blog->title }}&url={{ url()->current() }}"><i class="fab fa-twitter"></i></a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        ...
                        @if ($previousBlog)
                            <li>
                                <a href="{{ route('blogs.details', $previousBlog->slug) }}">
                                    ...
                                    <p>{{ truncateTitle($previousBlog->title, 15) }}
                        ...
                        @if ($nextBlog)
                            <li>
                                <a href="{{ route('blogs.details', $nextBlog->slug) }}">
                                    <p>{{ truncateTitle($nextBlog->title, 15) }}
                        ...
                        @foreach ($comments as $comment)
                            <div class="fp__single_comment m-0 border-0">
                                ...
                                <div class="fp__single_comm_text">
                                    <h3>{{ $comment->user->name }}<span>{{ date('d-M-Y | H:i', strtotime($comment->created_at)) }}</span></h3>
                                    <p>{{ $comment->comment }}</p>
                                </div>
                            </div>
                        @endforeach
                            ...
                            <div class="row">
                                <div class="col-xl-12">
                                    ...
                                    <div class="fp__contact_form_input textarea">
                                        ...
                                        <textarea id="text" rows="5" placeholder="Your Comment" name="comment"></textarea>
                                    </div>
                                    <button id="comment" type="submit" class="common_btn mt_20" disabled="disabled">Submit comment</button>
                                </div>
                            </div>
                        </form>
                    </div>
                            ...
                            <ul>
                                @foreach ($latestBlogs as $latestBlog)
                                    <li>
                                        ...
                                        <div class="text">
                                            <a href="{{ route('blogs.details', $latestBlog->slug) }}">{!! truncateTitle($latestBlog->title, 15) !!}</a>
                                            <p><i class="far fa-calendar-alt"></i>{{ date('d m Y | H:s', strtotime($latestBlog->created_at)) }}</p>
                                        </div>
                                    </li>
                                @endforeach
                            </ul>
