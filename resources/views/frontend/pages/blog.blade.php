                    ...
                    <div class="col-xl-4 col-md-4">
                        <select class="nice-select" name="category" id="category">
                            ...
                            @foreach ($categories as $category)
                                <option @selected(@request()->category === $category->slug) value="{{ $category->slug }}">{{ $category->title }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-xl-2 col-md-3">
                        <button type="submit" class="common_btn" id="search-btn">search</button>
                    </div>
                </div>
            </form>
            <div class="row">
                    ...
                    <div class="col-xl-4 col-sm-6 col-lg-4 wow fadeInUp" data-wow-duration="1s">
                        <div class="fp__single_blog">
                            ...
                            <div class="fp__single_blog_text">
                                <a class="category" href="#">{{ $blog->category->title }}</a>
                                ...
                                <a class="title" href="{{ route('blogs.details', $blog->slug) }}">{!! truncateTitle($blog->title, 25) !!}</a>
                            </div>
                        </div>
                    </div>
                @endforeach
                @if ($blogs->isEmpty())
                    <h5 class="text-center">No Blogs Found For This Category!</h5>
                @endif
            </div>
            @if ($blogs->hasPages())
